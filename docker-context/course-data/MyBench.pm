...
sub fork_and_work($$)
{
    ...
    while ($num_kids < $kids_to_fork)
    {
        ...
        elsif (defined $pid)
        {
            ...
            ## return results
            if ($result[3] and $result[5]) {print $pipe "@result\n";}
            $pipe->close();
            exit 0;
        }
        else
...
sub compute_results(@)
{
    ...
    while (@_)
    {
        ...
        my $rec = shift; chomp $rec unless !$rec;
        my ($id, $cnt, $min, $max, $avg, $tot) = split /\s+/, $rec unless !$rec;
...
{
    my $tot;
    for (@_) { $tot += $_ unless !$_; }
    return $tot / @_ unless !@_;
	return 0;
}
